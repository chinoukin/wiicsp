 ...
 **/
@TimerSchedule
public class SerListMageConfirmTaskService extends DisallowConcurrentJob implements QuartzJob {
    ...
    public QuartzJobOptions getOption() {
        if (ConverterUtil.isEmpty(option)) {
            option = new QuartzJobOptions("confirmSerListMageTask", QuartzManager.JOB_GROUP, this.getClass());
            // 设置任务说明
            option.setDescription("服务单服务中到期自动确认完成");
            // 设置触发器名称
            option.setTriggerName("confirm_serListMageTask");
            // 设置触发器分组
            option.setTriggerGroupName(QuartzManager.TRIGGER_GROUP);
            // 设置触发器说明
            option.setTriggerDescription("每隔一小时执行一次【服务单服务中到期自动确认】");
            // 设置CRON表达式
            option.setCron("0 0 */1 * * ?");
            // 立即执行
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        logger.debug("【服务单确认定时任务】定时任务 开始执行:" + ConverterUtil.dateToString(new Date(), ConverterUtil.FORMATE_DATE_TIME_24H));
        try {
            ...
            serListMageService.confirmSerListMageTask();
        } catch (Exception e){
            logger.error("exception in TestTask", e);
        }

        logger.debug("【服务单确认定时任务】定时任务 执行完成:" + ConverterUtil.dateToString(new Date(), ConverterUtil.FORMATE_DATE_TIME_24H));
