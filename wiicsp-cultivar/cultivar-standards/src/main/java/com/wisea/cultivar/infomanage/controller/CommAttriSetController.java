...
import com.wisea.cultivar.common.vo.product.InsertCommAttriSetVo;
import com.wisea.cultivar.infomanage.service.CommAttriSetService;
...
@RestController
@PreAuthorize("hasAuthority('im:gas')")
...
public class CommAttriSetController {
    ...
    @DataCheck
    public ResultPoJo commAttriSetAdd(@RequestBody ZhfwCommAttriSet po) {
    ...
    @DataCheck
    public ResultPoJo modifyCommAttriSet(@RequestBody ZhfwCommAttriSet po) {
    ...
    @DataCheck
    public ResultPoJo delCommAttriSet(@RequestBody ZhfwCommAttriSet po) {
    ...
    public ResultPoJo delAll(@RequestBody DelAllPo po) {
        return commAttriSetService.delAll(po);
    }
    /**
    ...
    @ApiOperation(value = "添加/编辑之前需要查询商品分类和属性列表",
            notes = "请求参数是：;返回两个集合")
    @DataCheck
    public ResultPoJo<InsertCommAttriSetVo> beforeInsert(@RequestBody CommAttriSetPagePo po) {
        ResultPoJo<InsertCommAttriSetVo> insertCommAttriSetVoResultPoJo =
                commAttriSetService.beforeInsert(po.getPid());
