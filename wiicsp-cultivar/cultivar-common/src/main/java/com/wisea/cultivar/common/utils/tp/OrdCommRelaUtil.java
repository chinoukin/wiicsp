...
import com.wisea.cultivar.common.dao.tp.SellerSpeSerCostMageMapper;
...
import com.wisea.cultivar.common.entity.tp.OrdCommRelaPlatfCost;
...
public class OrdCommRelaUtil {
    ...
    public static LtaCommRelaMapper ltaCommRelaMapper = SpringBootContext.getBean(LtaCommRelaMapper.class);
     ...
     * @Description 计算长期协议订单商品服务费
     **/
    public static List<LtaOrdCommRela> ltaPlatfCost(List<LtaOrdCommRela> ltaOrdCommRelas, Long sellerId, Long buyerId) {
        // 发布商品ID
        List<Long> commRelaIds = ltaOrdCommRelas.stream().map(e -> {
            return e.getLtaCommRelaId();
        }).collect(Collectors.toList());
        // 根据发布商品ID查询对应品类ID   key: 发布商品ID，value：品类ID
        Map<Long, Long> commIdMap = ltaCommRelaMapper.getCommIdMap(commRelaIds);

        // 封装公共参数
        List<OrdCommRelaPlatfCost> ordCommRelaPlatfCosts = Lists.newArrayList();
        ltaOrdCommRelas.forEach(e->{
            OrdCommRelaPlatfCost ordCommRelaPlatfCost = new OrdCommRelaPlatfCost();
            ordCommRelaPlatfCost.setOrdCommRelaId(e.getId());
            ordCommRelaPlatfCost.setCommPubId(e.getLtaCommRelaId());
            ordCommRelaPlatfCost.setPrice(e.getPrice());
            ordCommRelaPlatfCost.setCount(e.getSlNum());
            ordCommRelaPlatfCost.setCommId(commIdMap.get(e.getLtaCommRelaId()));
            ordCommRelaPlatfCosts.add(ordCommRelaPlatfCost);
        });

        // 调用计算服务费方法
        calculatePlatfCost(ordCommRelaPlatfCosts, "2", sellerId, buyerId);
     ...
     **/
    public static List<OrdCommRela> ordPlatfCost(List<OrdCommRela> ordCommPubInfos, String commPubType, Long sellerId, Long buyerId) {
        ...
        // 调用计算服务费方法
        calculatePlatfCost(ordCommRelaPlatfCosts, commPubType, sellerId, buyerId);
     ...
     **/
    public static OrdCommRela commPlatfCost(OrdCommRela ordCommPubInfo, String commPubType, Long sellerId, Long buyerId) {
        List<OrdCommRela> ordCommPubInfos = Lists.newArrayList();
        ordCommPubInfos.add(ordCommPubInfo);
        ordPlatfCost(ordCommPubInfos, commPubType, sellerId, buyerId);
     ...
     **/
    public static List<OrdCommRelaPlatfCost> calculatePlatfCost(List<OrdCommRelaPlatfCost> ordCommRelaPlatfCosts, String commPubType, Long sellerId, Long buyerId) {
        // 判断买卖家是否设置了特殊交易服务费
        ...
        if(ConverterUtil.isEmpty(sellerSpeSerCostMage)){
            ...
            ordCommRelaPlatfCosts.forEach(e->{
                ...
                // 判断商家该商品是否有优惠服务费
                YhSerChargeMage yhSerChargeMage = SerChargeMageUtils.getYhSerChargeMage(e.getCommId(), commPubType, sellerId);
                ...
                if(ConverterUtil.isEmpty(yhSerChargeMage) || yhSerChargeMage.getDiscountType().equals(DictConstants.DISCOUNT_TYPE_0)){
                    ...
                    MrSerChargeMage mrSerChargeMage = SerChargeMageUtils.getMrSerChargeMage(e.getCommId(), commPubType);
