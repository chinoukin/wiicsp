 ...
 */
@TimerSchedule
public class ReconExamInvoSettleTask extends DisallowConcurrentJob implements QuartzJob {
    ...
    public QuartzJobOptions getOption() {
        if (ConverterUtil.isEmpty(option)) {
            option = new QuartzJobOptions("ReconExamInvoSettleTask", QuartzManager.JOB_GROUP, this.getClass());
            // 设置任务说明
            option.setDescription("自动验票定时任务");
            // 设置触发器名称
            option.setTriggerName("Recon_ExamInvo_Task_every_minute");
            // 设置触发器分组
            option.setTriggerGroupName(QuartzManager.TRIGGER_GROUP);
            // 设置触发器说明
            option.setTriggerDescription("每分钟执行一次【自动验票定时任务】");
            // 设置CRON表达式
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        logger.debug("【自动验票定时任务】定时任务 开始执行:" + ConverterUtil.dateToString(new Date(), ConverterUtil.FORMATE_DATE_TIME_24H_MLINE));
        if (ConverterUtil.isEmpty(service)) {
            service = SpringBootContext.getBean(SettleBillService.class);
        }
        //自动对账定时任务
        ResultPoJo<Object> resultPoJo = service.findExamInvoSettle();
        context.setResult(resultPoJo);
        logger.debug("【自动验票定时任务】定时任务 执行结束:{}", ConverterUtil.dateToString(new Date(), ConverterUtil.FORMATE_DATE_TIME_24H_MLINE));
    }
}
