package com.wisea.transac.tp.system.utils;
...
public class TpSystemMembUtils {
    ...
    public static List<String> getMonthDayList(OffsetDateTime date){
    	...
    	for(int i = 1; i <= getMonthAllDay(date); i++){
    		list.add(yyyymm+"-"+String.format("%02d",i));
    	}
    	return list;
    }
     ...
     * @version 1.0
     */
    public static OffsetDateTime stringToYyyyMmDd(String str){
    	if(ConverterUtil.isEmpty(str)){
    		return null;
    	}
		if(str.length() <= 4){
			return ConverterUtil.stringToOffsetDateTime(str + "-01-01");
			...
			if(arr[1].length() > 1){
				...
				data = arr[0] + "-0" + arr[1] + "-01";
			}
			return ConverterUtil.stringToOffsetDateTime(data);
		}else{
			return ConverterUtil.stringToOffsetDateTime(str);
		}
    }
}
