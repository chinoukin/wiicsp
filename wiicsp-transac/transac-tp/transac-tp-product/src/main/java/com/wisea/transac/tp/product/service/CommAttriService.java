...
public class CommAttriService {
    ...
    public ResultPoJo addCommAttri(CommAttriMage po) {
     ...
     * @return
     */
    private StringBuffer getDictValueStringBuffer(CommAttriMage po, StringBuffer buffer, String str) {
        //首先得到每个option
     ...
     * 编辑商品属性
     */
    public ResultPoJo<CommAttriMage> editCommAttri(CommAttriMageGetPo po) {
        ResultPoJo<CommAttriMage> pojo = new ResultPoJo<>(ConstantCodeMsg.NOMAL);
        //如果属性已经与商品关联了，那么就不能进行修改
        List<CommAttriSet> commAttriSets = commAttriSetMapper.selectCommAttriById(po.getId());
        if (commAttriSets.size() > 0) {
            pojo.setCode(ConstantCodeMsg.ERR_312);
            pojo.setMsg(ConstantCodeMsg.MSG_312);
            return pojo;
        }
        CommAttriMage commAttriMage = commAttriMageMapper.selectByPrimaryKey(po.getId());
        pojo.setResult(commAttriMage);
        return pojo;
    }
     ...
     */
    @Transactional(readOnly = false, rollbackFor = Exception.class)
    public ResultPoJo modifyCommAttri(CommAttriMage po) {
     ...
     */
    @Transactional(readOnly = false, rollbackFor = Exception.class)
    public ResultPoJo delCommAttri(CommAttriMage po) {
     ...
     * 属性字段的唯一验证
     */
    public ResultPoJo getAttriColumnOnlyState(CommAttriMage po) {
