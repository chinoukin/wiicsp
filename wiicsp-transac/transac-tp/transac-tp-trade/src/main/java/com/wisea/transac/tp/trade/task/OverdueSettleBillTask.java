 ...
 **/
@TimerSchedule
public class OverdueSettleBillTask extends DisallowConcurrentJob implements QuartzJob {
    ...
    public QuartzJobOptions getOption() {
        if (ConverterUtil.isEmpty(option)) {
            option = new QuartzJobOptions("OverdueSettleBillTask", QuartzManager.JOB_GROUP, this.getClass());
            // 设置任务说明
            option.setDescription("逾期结算单定时任务");
            // 设置触发器名称
            option.setTriggerName("Overdue_Settle_Bill_Task_every_day");
            // 设置触发器分组
            option.setTriggerGroupName(QuartzManager.TRIGGER_GROUP);
            // 设置触发器说明
            option.setTriggerDescription("每天0点执行一次【逾期结算单定时任务】");
            // 设置CRON表达式
            option.setCron("10 0 0 */1 * ?"); //每天0点执行
            // 立即执行
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        logger.debug("【逾期结算单】定时任务 开始执行:" + ConverterUtil.dateToString(new Date(), ConverterUtil.FORMATE_DATE_TIME_24H_MLINE));
        if (ConverterUtil.isEmpty(service)) {
            service = SpringBootContext.getBean(SettleBillService.class);
        }
        //逾期结算单定时任务
        ResultPoJo<List<SettleBill>> resultPoJo = service.overdueSettleBillTask();
        context.setResult(resultPoJo);
        logger.debug("【逾期结算单】定时任务 执行结束:{}", ConverterUtil.dateToString(new Date(), ConverterUtil.FORMATE_DATE_TIME_24H_MLINE));
    }
}
